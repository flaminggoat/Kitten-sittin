...
namespace MyUILibrary
{
    ...
    public class RadialProgress : VisualElement
    {
        public new class UxmlTraits : VisualElement.UxmlTraits
        {
            ...
            UxmlFloatAttributeDescription m_MaxAttribute = new UxmlFloatAttributeDescription()
            {
                name = "max"
            };

            UxmlFloatAttributeDescription m_ValueAttribute = new UxmlFloatAttributeDescription()
            {
                name = "value"
            };
            ...
            public override void Init(VisualElement ve, IUxmlAttributes bag, CreationContext cc)
            {
                ...
                (ve as RadialProgress).max = m_MaxAttribute.GetValueFromBag(bag, cc);
                (ve as RadialProgress).value = m_ValueAttribute.GetValueFromBag(bag, cc);
            }
        }
        ...
        public float value
        {
            // The value property is exposed in C#.
            get => m_Value;
            set
            {
                ...
                m_Value = value;
                m_Label.text = Mathf.Clamp(Mathf.Round(value), 0, Mathf.Round(m_Max)).ToString();
                MarkDirtyRepaint();
            }
        }

        // This is the max value that will be displayed
        float m_Max;
        ...
        public float max
        {
            ...
            set
            {
                // Whenever the progress property changes, MarkDirtyRepaint() is named.
                m_Max = value;
                MarkDirtyRepaint();
            }
        }
        ...
        void DrawMeshes(MeshGenerationContext context)
        {
            ...
            // Keep progress between 0 and max
            float clampedProgress = Mathf.Clamp(m_Value, 0.0f, m_Max);

            // Determine how many triangles are used to depending on progress, to achieve a partially filled circle
            int sliceSize = Mathf.FloorToInt((k_NumSteps * clampedProgress) / m_Max);
